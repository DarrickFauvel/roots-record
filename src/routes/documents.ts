...
documentsRouter.post("/", upload.single("file"), async (req, res) => {
  ...
  const { public_id, secure_url } = await uploadDocument(req.file.buffer, {
    folder: `roots-record/${person_id}`,
  });
...
documentsRouter.delete("/:id", async (req, res) => {
  const result = await db.execute({
    sql: "SELECT cloudinary_public_id, person_id FROM documents WHERE id = ?",
    args: [req.params.id],
  });
