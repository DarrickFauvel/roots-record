...
peopleApiRouter.put("/:id", async (req, res) => {
  ...
  await db.execute({
    ...
    args: [given_name.trim(), surname?.trim() || null, sex || "unknown", birth_date || null, birth_place?.trim() || null,
           death_date || null, death_place?.trim() || null, death_cause?.trim() || null, notes?.trim() || null, req.params.id],
  });
...
peopleApiRouter.delete("/:id", async (req, res) => {
  await db.execute({ sql: "DELETE FROM people WHERE id = ?", args: [req.params.id] });
...
// GET /api/people/:id/relationships — SSE patch relationships section
peopleApiRouter.get("/:id/relationships", async (req, res) => {
  ...
  const html = await eta.renderAsync("partials/relationships", {
    ...
    siblings: siblings.rows, personId: req.params.id, allPeople: allPeople.rows,
  });
...
// GET /api/people/:id/residences — SSE patch residences section
peopleApiRouter.get("/:id/residences", async (req, res) => {
  const result = await db.execute({
    sql: "SELECT * FROM residences WHERE person_id = ? ORDER BY start_date",
    args: [req.params.id],
  });
  const html = await eta.renderAsync("partials/residences", {
    residences: result.rows, personId: req.params.id,
  });
  ServerSentEventGenerator.stream(req, res, (stream) => {
...
// GET /api/people/:id/documents — SSE patch documents section
peopleApiRouter.get("/:id/documents", async (req, res) => {
  const result = await db.execute({
    sql: "SELECT * FROM documents WHERE person_id = ? ORDER BY created_at DESC",
    args: [req.params.id],
  });
  const html = await eta.renderAsync("partials/document-list", {
    documents: result.rows, personId: req.params.id,
  });
  ServerSentEventGenerator.stream(req, res, (stream) => {
